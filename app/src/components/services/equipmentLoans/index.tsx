...
export class EquipmentReservations extends React.Component<props, state> {
  ...
  checkTimeFrame() {
    ...
    const hours = moment
      .duration(
        moment(this.state.toDate + " " + this.state.toTime).diff(
          moment(this.state.fromDate + " " + this.state.fromTime)
        )
      )
      .asHours();
    ...
    if (hours > 72) {
      ...
      this.setState({
        throwTimeError: false,
        availableItems: getAvailableEquipment(
          this.state,
  ...
  addRemoveItem(type, flavor) {
    ...
    const { selectedTypes, availableItems, items } = this.state;

    if (!selectedTypes.includes(type)) {
      // add item
      // get an item of corresponding item type
      const item: any = availableItems.find(
        item => item.itemType == type || item.item == type
      );
  ...
  async submit() {
    ...
    if (success == true) {
      this.props.newMessage(
        "Success!<br/>Your reservation ID is " +
          this.state.reservationID +
          "<br/>Please bring your reservation ID when picking up your equipment"
      );
    } else {

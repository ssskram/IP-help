...
export class NewPC extends React.Component<props, any> {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.machineType != this.state.machineType ||
    ...
    if (
      !prevState.accessories.some(e => e.value === "Docking Station") &&
      this.state.accessories.some(e => e.value === "Docking Station")
    ) {
  ...
  setAccessories() {
    this.setState({ accessories: [] }, () => {
      ...
      if (this.state.machineType.value === "Desktop") {
        this.setState(
          { availableAccessories: Selects.DesktopAccessories },
          () => this.addSpeakers()
        );
      }
      if (this.state.machineType.value === "Laptop") {
        this.setState({ availableAccessories: Selects.LaptopAccessories }, () =>
          this.addSpeakers()
        );
      }
      ...
      if (
        this.state.machineType.value === "Zero client" ||
        this.state.machineType === "Thin client"
      ) {
        this.setState(
          { availableAccessories: Selects.ZeroClientAccessories },
          () => this.addSpeakers()
        );
      }
    });
  ...
  addSpeakers() {
    ...
    if (
      this.state.department.value === "Police" &&
      this.state.machineType.value === "Desktop"
    ) {
      var join = this.state.availableAccessories.concat(
        Selects.PoliceAccessories
      );
      this.setState({
        availableAccessories: join,
        accessories: Selects.PoliceAccessories
      });
    }
  }
  ...
  async submit() {
    let load = {
      ...
      employeeStartDate:
        moment(this.state.employeeStartDate).format("MM/DD/YYYY") || "",
    ...
    return (
      <div className="col-md-12">
        ...
        <Modal
          ...
          center
        >
          {this.state.accessoriesTooltip === true && <StandardAccessories />}
          {this.state.softwareTooltip === true && <StandardSoftware />}
        </Modal>
      </div>
    );
  }
